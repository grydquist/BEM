...
% Points, normal vectors, Jacobian at colloc points
...
%               Tx rotates the Gauss point to (0,0,1), so we need the
...
%                       Only need to calc B once per mode
                        if(n==0)
...
%       Forcing integral calc'd for a mode point! Put in B
        b(row:row+2) = b(row:row+2) + bt*dphi - 4*pi*Uc;
    end
end

%% Outer, Galerkin integral (done over a sphere! uses points in previous section)
...
%                       Integrate with that value
                        At = At + v*Y(i,j)*wg(i)*dphi;
                        
%                       Integrate b now
                        if n2 == 0
                            bt = bt + b(3*ii-2:3*ii)*Y(i,j)*wg(i)*dphi;
                        end
                    end
                end
...
% Singular value decomposition here
[Us,S,V] = svd(A2);
Si = 1./S;
Si(Si>1e10)=0;
