...
%       point at current theta/phi
        Ycr = SpHarmTNew(n,0,0);
        xcr(1) = SpHReconst(x1mnr,Ycr);
        xcr(2) = SpHReconst(x2mnr,Ycr);
        xcr(3) = SpHReconst(x3mnr,Ycr);
        xcr = reshape(real(xcr),[3,1]);
%         xcr = x(:,i,j);
...
%               Gauss point in rotated reference frame
                gp = [sin(thtf(i2))*cos(phif(j2)); sin(thtf(i2))*sin(phif(j2)); cos(thtf(i2))];
...
%                       Only need to calc B once per colloc point
                        if(n==0)
                            ...
                            v2= Gij(r2);
%                             v= Tij(r,nkg2(:,ig,jg));
%                             lglg(ig,jg) = dot(r,nkg2(:,ig,jg))/norm(r)^2;
%                             lglg(ig,jg) = v(1,1)*norm(r);
                            tmptmp(ig,jg) = v2(1,1);
                            bt = bt + v*Jg(ig,jg)*ws(ig); %!!!!!!!!!!!! SEE DIFFERENCE (no forcing)
                            bt2 = bt2 + v*Jg(ig,jg)*wg(ig)/sin(tht(ig))*dphi;
                            bt3 = bt3 + v2*J(ig,jg)*wg(ig)/sin(tht(ig))*dphi;
                        end
                    end
                end
