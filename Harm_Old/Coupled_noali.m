...
%       Curvature derivatives        
        kt = -2*Jt./J(i,j).*k + 0.5*J(i,j).^-2. ... 
                * (Et.*N + Nt.*E - 2*Ft.*M - 2*Mt.*F + Gt.*L + Lt.*G);
        kp = -2*Jp./J(i,j).*k + 0.5*J(i,j).^-2. ... 
...
%       Moments and their needed partials
        mab = (k - kR(i,j))*gn;
        dmab(1,1) = (kt - kdR(1,i,j))*gn(1,1) + (k - kR(i,j))*dgt(1,1);
        dmab(1,2) = (kt - kdR(1,i,j))*gn(1,2) + (k - kR(i,j))*dgt(1,2);
        dmab(2,1) = (kp - kdR(2,i,j))*gn(2,1) + (k - kR(i,j))*dgp(2,1);
        dmab(2,2) = (kp - kdR(2,i,j))*gn(2,2) + (k - kR(i,j))*dgp(2,2);
...
scatter3(trcpnt(cts),real(SpHReconst(x2mn,Ytrc)),real(SpHReconst(x3mn,Ytrc)),'ro');
