...
%       Rotation Matrix
        t1 = [cos(phif(j)),-sin(phif(j)),0;sin(phif(j)),cos(phif(j)),0;0,0,1];
        t2 = [cos(-thtf(i)),0,sin(-thtf(i));0,1,0;-sin(-thtf(i)),0,cos(-thtf(i))];
        t3 = [cos(-phif(j)),-sin(-phif(j)),0;sin(-phif(j)),cos(-phif(j)),0;0,0,1]; 
        Tx = t1*t2*t3;
...
%       instead of each individually. Would REALLY cut cost
        x1mnr = SpHRot(x1mn,phif(j),-thtf(i),0-phif(j), myfacs);
        x2mnr = SpHRot(x2mn,phif(j),-thtf(i),0-phif(j), myfacs);
        x3mnr = SpHRot(x3mn,phif(j),-thtf(i),0-phif(j), myfacs);
...
%         bb(row:row+2) = bb(row:row+2) + bt*dphi/mu/(1+lam); %!!!!!!!!!!!!!!!!!!!!!!!!!
        bn(row:row+2) = bn(row:row+2) + bt*dphi/mu/(1+lam) - Uc*8*pi/(1+lam);
    end
end

% Filter stuff out
b1t = real(reshape(bn(1:3:end),npf,ntf))';
b2t = real(reshape(bn(2:3:end),npf,ntf))';
b3t = real(reshape(bn(3:3:end),npf,ntf))';

b1S = SpT(Ytf,b1t,thtf,phif,p);
b2S = SpT(Ytf,b2t,thtf,phif,p);
b3S = SpT(Ytf,b3t,thtf,phif,p);



b(1:3:end) = real(SpHReconst(b1S,Yt,p))';
b(2:3:end) = real(SpHReconst(b2S,Yt,p))';
b(3:3:end) = real(SpHReconst(b3S,Yt,p))';
...
% some output
disp([max(abs(x3mn-ix3mn)), real(u3(1)), max(max(squeeze(ua11(1,:,:)))),max(max(max(abs(myf(:,:,:))))),cts]);
