%   EXPERIMENTAL, NOTHING REALLY OF USE HERE
...
for i = 1:nt
    for j = 0:gf*p
        ws(i) = ws(i) + legendreP(j,xs(i))*sin(tht(i)/2)*2;
        ws2(i)= ws2(i)+ legendreP(j,xs(i))/cos(tht(i)/2);
    end
    ws(i) = ws(i)*wg(i);
    ws2(i) = ws2(i)*wg(i);
end
...
allfun1 = SpTNew(mylegs,squeeze(myf(1,:,:)).*J./sin(thf),wgf,dphif);
allfun2 = SpTNew(mylegs,squeeze(myf(2,:,:)).*J./sin(thf),wgf,dphif);
allfun3 = SpTNew(mylegs,squeeze(myf(3,:,:)).*J./sin(thf),wgf,dphif);
...
%         x3mnr = SpHRot(x3mn,phi(j),-tht(i),0, myfacs);
        ...
        allfunr1 = SpHRot(allfun1,phi(j),-tht(i),0-phi(j), myfacs);
        allfunr2 = SpHRot(allfun2,phi(j),-tht(i),0-phi(j), myfacs);
        allfunr3 = SpHRot(allfun3,phi(j),-tht(i),0-phi(j), myfacs);
...
%         Nmyf(3,:,:) = SpHReconst(myfS3,Ypt);
        Nmyf(1,:,:) = SpHReconst(allfunr1,Yt);
        Nmyf(2,:,:) = SpHReconst(allfunr1,Yt);
        Nmyf(3,:,:) = SpHReconst(allfunr1,Yt);
...
%                       Only need to calc B once per colloc point
                        if(n==0)
                            r = xcr-xcg(:,ig,jg);
                            v = Gij(r);
%                             bt = bt + v*Nmyf(:,ig,jg)*Jg(ig,jg)/sin(tht(ig))*ws(ig);
                            bt = bt + v*Nmyf(:,ig,jg)*ws(ig);
                        end
                    end
                end
