...
ws = zeros(1,length(wgf));
for i = 1:ntf
    for j = 0:gf*q
        ws(i) = ws(i) + legendreP(j,xsf(i))/cos(thtf(i)/2);
    end
    ws(i) = ws(i)*wgf(i);
end
...
%       Nonrotated locations of rotated grid points.        
        xcg(1,:,:) = SpHReconst(x1mnr,Ytf,p);
        xcg(2,:,:) = SpHReconst(x2mnr,Ytf,p);
        xcg(3,:,:) = SpHReconst(x3mnr,Ytf,p);
        xcg = real(xcg);
...
%       Filter down area element
%         JgS = SpT(Ytf,Jg,thf,phf,p);
%         Jga = real(SpHReconst(JgS,Ypt));
