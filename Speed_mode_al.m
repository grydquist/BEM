...
x = zeros(3,nt,np);
x(1,:,:) = SpHReconst(x1mn,Yt,p);
x(2,:,:) = SpHReconst(x2mn,Yt,p);
x(3,:,:) = SpHReconst(x3mn,Yt,p);
...
%       Nonrotated locations of rotated grid points.        
        xcg(1,:,:) = SpHReconst(x1mnr,Yt,p);
        xcg(2,:,:) = SpHReconst(x2mnr,Yt,p);
        xcg(3,:,:) = SpHReconst(x3mnr,Yt,p);
        xcg = real(xcg);
...
%               Gauss point in rotated reference frame
                gpf = [sin(thtf(i2))*cos(phif(j2)); sin(thtf(i2))*sin(phif(j2)); cos(thtf(i2))];
...
%               transpose.
                gpf = Tx'*gpf;
                phitf = atan2(gpf(2),gpf(1));
%               Because it can sometimtes be gp(3) - 1 = 2e-16
                if gpf(3)> 1; gpf(3) =  1;end
                if gpf(3)<-1; gpf(3) = -1;end
                thetf = acos(gpf(3));
                
%               We need a fine thet and coarse thet. Distinction made here
                if(i2<nt+1 && j2<np+1)
...
%               Inward normal
                nkgf(:,i2,j2) = real(cross(dxtg,dxpg));
                nkgf(:,i2,j2) = -nkgf(:,i2,j2)./norm(nkgf(:,i2,j2));
                
%               Jacobian (area element) via fundamental forms
                Jgf(i2,j2) = real(sqrt(dot(dxtg,dxtg) ... 
                          * dot(dxpg,dxpg) ...
                          - dot(dxtg,dxpg)^2));
            end
        end
...
%       Fine grid constants
        JgC = SpT(Ytf,Jgf,thf,phf);
        
%       Filter out high modes        
        Jg = SpHReconst(JgC,Yt,p);
        
%       Normal vector: interpolate to high order
        n1C = SpT(Ytf,squeeze(nkgf(1,:,:)),thf,phf);
        n2C = SpT(Ytf,squeeze(nkgf(2,:,:)),thf,phf);
        n3C = SpT(Ytf,squeeze(nkgf(3,:,:)),thf,phf);
        nkg(1,:,:) = SpHReconst(n1C,Yt,q);
        nkg(2,:,:) = SpHReconst(n2C,Yt,q);
        nkg(3,:,:) = SpHReconst(n3C,Yt,q);
...
%       This was mewt with massive failure when I tried it though.
        Nmyf(1,:,:) = SpHReconst(myfS1,Ypt,p);
        Nmyf(2,:,:) = SpHReconst(myfS2,Ypt,p);
        Nmyf(3,:,:) = SpHReconst(myfS3,Ypt,p);
...
ua11(1,:,:) = real(SpHReconst(u1,Yt,p));
ua11(2,:,:) = real(SpHReconst(u2,Yt,p));
ua11(3,:,:) = real(SpHReconst(u3,Yt,p));
...
u2 = ut(2:3:end);
u3 = ut(3:3:end);
ua(1,:,:) = real(SpHReconst(u1,Yt,p));
ua(2,:,:) = real(SpHReconst(u2,Yt,p));
ua(3,:,:) = real(SpHReconst(u3,Yt,p));
...
% New x's
x(1,:,:) = real(SpHReconst(x1mn,Yt,p));
x(2,:,:) = real(SpHReconst(x2mn,Yt,p));
x(3,:,:) = real(SpHReconst(x3mn,Yt,p));
...
% Force reconstructys
myf21 = real(SpHReconst(myfS1,Yr,p));
myf22 = real(SpHReconst(myfS2,Yr,p));
myf23 = real(SpHReconst(myfS3,Yr,p));
