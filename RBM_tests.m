...
%================================Volume control
%================================ not sure wqhy needed
V = VolHarm(x,dxt,dxp,wg,tht,dphi);
if(tt==1)
    V2=  V;
end
x1mn = x1mn*(V2/V)^(1/3);
...
V2 = VolHarm(x,dxt,dxp,wg,tht,dphi);
...
Ut = [u12(1);u22(1);u32(1)];
Ot = [u12(2);u22(2);u32(2)];

disp(real([tt, Ot(2), norm(x1mn + x2mn + x3mn)]))
f1 = SpHReconst(u1,Yt);
f2 = SpHReconst(u2,Yt);
f3 = SpHReconst(u3,Yt);
...
% x2mn = x2mn + uu2*dt;
% x3mn = x3mn + uu3*dt;
x1mn = SpT(Yt,squeeze(x(1,:,:)),th,ph);
x2mn = SpT(Yt,squeeze(x(2,:,:)),th,ph);
x3mn = SpT(Yt,squeeze(x(3,:,:)),th,ph);
