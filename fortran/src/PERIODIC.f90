...
! Can be used to interpolate back to grid. Done for double layer
SUBROUTINE HtCell(Ht, info, x0, f, TG)
    ...
    IF(TG .ne. 1 .and. TG .ne. 2) THEN
        print *, "Option for TG must be 1 or 2 for double or single layer, respectively"
        stop
    ENDIF
...
!!!!!! If we're doing harmonics, all of the f's will be the same per cell.
!!!!!! No need to recalc all per cell, so just do something like the following:
!! IF(size(x0) .ne. size(f))
!!  Get multiple, loop over it in spreading step below.
...
!               Branching point here on whether we want single or double layer integral
                IF(TG .eq. 2) THEN
                    B(:,:,1) = BG(k3, info%xi, kn, info%eta, info%eye)
                    Hht(1, i, j, k) = B(1,1,1)*Hh(1,i,j,k) &
                                    + B(1,2,1)*Hh(2,i,j,k) &
                                    + B(1,3,1)*Hh(3,i,j,k)
                    Hht(2, i, j, k) = B(2,1,1)*Hh(1,i,j,k) &
                                    + B(2,2,1)*Hh(2,i,j,k) &
                                    + B(2,3,1)*Hh(3,i,j,k)
                    Hht(3, i, j, k) = B(3,1,1)*Hh(1,i,j,k) &
                                    + B(3,2,1)*Hh(2,i,j,k) &
                                    + B(3,3,1)*Hh(3,i,j,k)
...
SUBROUTINE PeriodicTest(info)
    ...
    REAL(KIND = 8), ALLOCATABLE :: x(:,:,:,:), xv(:,:), Jtmp(:,:)
...
!   Sphere setup
    DO i = 1,nt
        DO j = 1,np
            x(1,1,i,j) = SIN(Y%tht(i))*COS(Y%phi(j))
