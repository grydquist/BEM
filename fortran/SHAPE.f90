...
FUNCTION newcell(p, fali) RESULT(cell)
    INTEGER, INTENT(IN) :: p, fali
    TYPE(cellType) :: cell
...
SUBROUTINE Fluidcell(cell)
    ...
    COMPLEX(KIND = 8) :: A(3*cell%Y%nt*cell%Y%np, 3*(cell%Y%p + 1)*(cell%Y%p + 1)), &
                         b(3*cell%Y%nt*cell%Y%np), At(3,3), bt(3), &
                         A2(3*(cell%Y%p + 1)*(cell%Y%p + 1), 3*(cell%Y%p + 1)*(cell%Y%p + 1)), &
    ...
    REAL(KIND = 8) :: dxtg(3), dxpg(3), Uc(3), t1(3,3), t2(3,3), t3(3,3), Tx(3,3), &
                      ...
                      eye(3,3) , xcg(3, cell%Y%nt, cell%Y%np)
...
! -------------------------------------------------------------------------!
! KFunctions to calculate the kernels
