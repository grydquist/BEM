...
% dU = shft'*[0,0,1;0,0,0;.0,0,0]*shft/4/pi;
...
%% First time step
% For secant, we need the previous value of velocity. Luckily, on the first
% time step, we know that the velocity of the membrane is just the
% velocity of the fluid, which makes things somewhat easier. So that's our
% initial xmn_old
x1mno = x1mn;
x2mno = x2mn;
x3mno = x3mn;

% Max secant iters (including first step to get comparative value)
maxit = 10;
...
% Implicit iteration loop: calc velocity for the newest estimate of pos
for iter = 1:maxit
    
% For the first iteration, we need to calculate the residual at the current
% time step for the old values of deformation to get a starting point

if(iter == 1)
%   Hold on to the current reference frame
    x1mntmp = x1mn;
    x2mntmp = x2mn;
    x3mntmp = x3mn;
    
%   Calculate at an old value    
    x1mn = x1mni2;
    x2mn = x2mni2;
    x3mn = x3mni2;
end

% Now if iter == 2, actually start secant method
if(iter == 2)
    x1mn = x1mntmp;
    x2mn = x2mntmp;
    x3mn = x3mntmp;
...
% Additionally, the first iter of the first time step is important, as it
% gets all the reference info, so we gotta get that figured out too.
if(iter == 1 && cts ==1)
    x1mn = x1mntmp;
    x2mn = x2mntmp;
    x3mn = x3mntmp;
...
%       Here come the stresses!        
        ...
        %       !!!!!!!!!! HACKY FIRST TIME STEP, needs fixing for implicit
                if(cts == 1 && iter == 1)
                    c1R(:,i,j,i2,j2) = c1;
                    c2R(:,i,j,i2,j2) = c2;
                    kR(i,j,i2,j2) = k;
...
ut = A2\b2;
%% Implicit residual calculations

u1 = ut(1:3:end);
u2 = ut(2:3:end);
u3 = ut(3:3:end);

% Calculate f(x), i.e. the funciton we're trying to zero
fx1n = x1mn - x1mno - dt*u1;
fx2n = x2mn - x2mno - dt*u2;
fx3n = x3mn - x3mno - dt*u3;

% First iteration: just calculating the residual at the current time step
% for the "old" value of deformation. Get this value and cycle.
if(iter == 1)
    fx1o = fx1n;
    fx2o = fx2n;
    fx3o = fx3n;
    
%   On the first time step, just do an explicit to get things going.
    if(cts == 1)
        x1mn = x1mn + u1*dt;
        x2mn = x2mn + u2*dt;
        x3mn = x3mn + u3*dt;
...
% Calculate the next iter
x1mnt = x1mn - fx1n.*(x1mn - x1mni2)./(fx1n - fx1o);
x2mnt = x2mn - fx2n.*(x2mn - x2mni2)./(fx2n - fx2o);
x3mnt = x3mn - fx3n.*(x3mn - x3mni2)./(fx3n - fx3o);
...
% We've found the new value of x1mn, update the old value
x1mni2 = x1mno;
x2mni2 = x2mno;
x3mni2 = x3mno;
...
quiver3(reshape(x(1,:,:),[1,numel(x(1,:,:))]),reshape(x(2,:,:),[1,numel(x(1,:,:))]),reshape(x(3,:,:),[1,numel(x(1,:,:))]),reshape(ua11(1,:,:),[1,numel(x(1,:,:))]),reshape(ua11(2,:,:),[1,numel(x(1,:,:))]),reshape(ua11(3,:,:),[1,numel(x(1,:,:))]),'k')
