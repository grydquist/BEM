...
%               SpHarms order n, degree m eval'd at integ points
                ...
                for ig = 1:nt
                    for jg = 1:np
%                       Integral addition                        
                        At = At + vT(:,:,ig,jg)*Y(ig,jg)*Jg(ig,jg)*ws(ig)*dphi;
%                       RHS here can be vectorized
                        
%                       Only need to calc B once per colloc point
                        if(n==0)
                            bt = bt + vG(:,:,ig,jg)*Nmyf(:,ig,jg)*Jg(ig,jg)*ws(ig);
                        end
                    end
                end
...
A2(3*p^2+1:3*(p+1)^2,:) = 0;
A2(3*p^2+1:3*(p+1)^2,(3*p^2+1:3*(p+1)^2)) = eye(3*(2*p+1));
...
pbaspect([1,.5,1])
hold on
scatter3(trcpnt(cts),real(SpHReconst(x2mn,Ytrc)),real(SpHReconst(x3mn,Ytrc)),'bo');
