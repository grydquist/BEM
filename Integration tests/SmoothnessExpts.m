...
tht2 = linspace(.0001,pi-.0001,nt); %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
[ph2,th2] = meshgrid(phi,tht2);
Yt2 = SpHarmTNew(p,th2,ph2);
...
myfS1= SpTNew(mylegs,squeeze(myf(1,:,:)),wgf,dphif);
myfS2= SpTNew(mylegs,squeeze(myf(2,:,:)),wgf,dphif);
myfS3= SpTNew(mylegs,squeeze(myf(3,:,:)),wgf,dphif);
...
%       Rotation Matrix
        ...
        t2 = [cos(-tht2(i)),0,sin(-tht2(i));0,1,0;-sin(-tht2(i)),0,cos(-tht2(i))];
...
%       instead of each individually. Would cut cost
%       I also don't need the last phi rotation, which is removed in the
%       commented part and also above for t1, t2 t3
%         x1mnr = SpHRot(x1mn,phi(j),-tht(i),0, myfacs);
%         x2mnr = SpHRot(x2mn,phi(j),-tht(i),0, myfacs);
%         x3mnr = SpHRot(x3mn,phi(j),-tht(i),0, myfacs);
        x1mnr = SpHRot(x1mn,phi(j),-tht2(i),0-phi(j), myfacs); % so the phis should maybe be reversed...
        x2mnr = SpHRot(x2mn,phi(j),-tht2(i),0-phi(j), myfacs);
        x3mnr = SpHRot(x3mn,phi(j),-tht2(i),0-phi(j), myfacs);
           
%       Nonrotated locations of rotated grid points.        
        xcg(1,:,:) = SpHReconst(x1mnr,Yt2);
        xcg(2,:,:) = SpHReconst(x2mnr,Yt2);%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        xcg(3,:,:) = SpHReconst(x3mnr,Yt2);
        xcg = real(xcg);
        
%       The location of the new pole, should just be location of
%       point at current theta/phi
        xcrt1 = SpHReconst(x1mn,Yt2);
        xcrt2 = SpHReconst(x2mn,Yt2);
        xcrt3 = SpHReconst(x3mn,Yt2);
        xcr(1) = xcrt1(1,1);
        xcr(2) = xcrt2(1,1);
        xcr(3) = xcrt3(1,1);
...
%               Gauss point in rotated reference frame
                gp = [sin(tht2(i2))*cos(phi(j2)); sin(tht2(i2))*sin(phi(j2)); cos(tht2(i2))];
...
%                       Only need to calc B once per colloc point
                        if(n==0)
                            ...
                            ll(ig,jg) = v(1,1)*Jg(ig,jg)/sin(tht2(ig))*2*sin(tht2(ig)/2);
%                             ll(ig,jg) = Jg(ig,jg)/sin(tht2(ig))*2*sin(tht2(ig)/2);
                        end
                    end
                end
